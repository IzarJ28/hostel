...
if(isset($_GET['email_confirmation']))
{
    ...
    $query = select("SELECT * FROM `user_reg` WHERE `email`=? AND `token`=? LIMIT 1 ",
    ...
    if(mysqli_num_rows($query)==1)
    {
       ...
       if($fetch['is_verified']==1){
        echo "<script>alert('Email already verified.')</script>";
          ...
          $update = update("UPDATE `user_reg` SET `is_verified`=?  WHERE `id`=?",[1,$fetch['id']],'ii');
          if ($update){
            echo "<script>alert('Email verification succesful!')</script>";
          }
          else{
