...
<h6 class="text-center bg-danger bg-gradient text-white p-3 m-0">Design and Develop By Team HAHAHA</h6>
...
register_form.addEventListener('submit', (e)=> {
    ...
    data.append('profile', register_form.elements['profile'].files[0]); // Updated to handle file input
    data.append('pass', register_form.elements['pass'].value);
    data.append('cpass', register_form.elements['cpass'].value);
    
    // Handle radio button value
    let clientType = register_form.elements['clientType'].value;
    ...
    xhr.onload = function() {
        if (this.responseText == 'pass_mismatch') {
            ...
            alert('error', "Only JPEG, PNG and WEBP are allowed!");
        } else if (this.responseText == 'upd_failed') {
            alert('error', "Image upload failed!");
        } else if (this.responseText == 'mail_failed') {
            alert('error', "Cannot send confirmation email, Server Down!");
        } else if (this.responseText == 'ims_failed') {
...
///login in before book
function checkLoginToBook(status, room_id, type_id, room_name) {
    if (status) {
        // Check room type ID and redirect accordingly with room name as a query parameter
        if (type_id === 8) {
            // Redirect to Function Rooms page
            window.location.href = 'function_room.php?id=' + room_id;
        } else if (type_id === 9) {
            // Redirect to Guest Rooms page with the room name as a query parameter
            window.location.href = 'guest_room.php?id=' + room_id + '&room_name=' + encodeURIComponent(room_name);
        } else {
            alert('error', 'Room type is not recognized!');
        }
    } else {
